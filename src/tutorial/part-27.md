...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/tutorial-image.jpg)
...
A cylinder is a poor approximation of a sphere, as it cannot represent the poles. But this hasn't stopped many games from using east&ndash;west wrapping to represent planetary maps. The polar regions are simply not part of the playable area.

What about wrapping north and south too?

If you wrap both east&ndash;west and north&ndash; south, you end up with the topology of a torus. So that's not a valid representation of a spherical body, although there are games that use that wrapping method. This tutorial only covers east&ndash;west wrapping, but you could add north&ndash;south wrapping as well, using the same approaches. It just requires more work and other metrics.
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/wrapping-maps/new-map-menu.png)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/wrapping-maps/grid-wrapping.png)
...
```
const int mapFileVersion = 5;</pre>
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/centering-the-map/columns.png)
...
We only have to adjust the map visualization when the center column index changes. So let's keep track of it in a field. Use a default value of &minus;1 and when creating a map, so new maps will always get centered.
...
[视频: centering-the-map/moving-left-right.mp4](https://catlikecoding.com/unity/tutorials/hex-map/part-27/centering-the-map/moving-left-right.mp4)

While we're still clamping the camera's movement, the map now tries to stay centered on the camera, teleporting chunk columns as needed. This is obvious when using a small map and a zoomed-out view, but on a large map the teleporting chunks are out of view of the camera. The original east&ndash;west edges of the map are only obvious because there is no triangulation between them yet.
...
[视频: centering-the-map/wrapping-camera.mp4](https://catlikecoding.com/unity/tutorials/hex-map/part-27/centering-the-map/wrapping-camera.mp4)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/gap.png)

Wrapping Neighbors

To triangulate the east&ndash;west connection we have to make the cells on the opposite sides of the map neighbors of each other. We're currently not doing this, because in HexGrid.CreateCell we only establish the E&ndash;W relationship with the previous cell if its X index is greater than zero. To wrap this relationship, we also have to connect the last cell of a row with the first of the same row, when wrapping is enabled.
...
With the E&ndash;W neighbor relationships established, we now get partial triangulation across the gap. The edge connection isn't perfect, because the perturbation doesn't tile correctly. We'll deal with that later.

![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/e-w.png)

We also have to wrap the NE&ndash;SW relationships. We can do this by connecting the first cell of each even row with last cells of the previous row. That's simply the previous cell.
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/ne-sw.png)

Finally, the wrapping SE&ndash;NW connections are established at the end of each odd row beyond the first. Those cells are to be connected to the first cell of the previous row.
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/se-nw.png)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/blending.png)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/blending-offset.png)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/incomplete-brush.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/missing-water-edge.jpg)
...
```
public int ColumnIndex { get; set; }</pre>
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/shore-edges.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/shore-complete.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/generated-without-wrapping.jpg)

![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/generated-with-wrapping.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/wrapping-region.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/terrain-seam.jpg)

![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/terrain-seam-detail.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/wrapped-without-erosion.jpg)

![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/connecting-east-and-west/wrapped-with-erosion.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/traveling-around-the-world/blocking-seam.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/traveling-around-the-world/feature-visibility-incorrect.jpg)
...
![](https://catlikecoding.com/unity/tutorials/hex-map/part-27/traveling-around-the-world/unit-on-wrong-size.jpg)
...
[视频: traveling-around-the-world/bouncing-across-map.mp4](https://catlikecoding.com/unity/tutorials/hex-map/part-27/traveling-around-the-world/bouncing-across-map.mp4)
...
[视频: traveling-around-the-world/wrapping-movement.mp4](https://catlikecoding.com/unity/tutorials/hex-map/part-27/traveling-around-the-world/wrapping-movement.mp4)

The final thing that we have to tweak is the initial rotation of the unit, when it faces the first cell it's going to travel to. If that cell happens to be on the other side of the east&ndash;west seam, then the unit ends up looking in the wrong direction.
...
I've set my project to use linear color space. Gamme space makes it much brighter.

I downloaded the final package and always generate the same map?

The generator is set to always use the same fixed seed, 1208905299, which is the one used for most screenshots. Disable Use Fixed Seed to make it random.
